...
if __name__ == "__main__":
    ...
    for epoch in range(args.start_epoch, args.start_epoch+Epoch):
        ...
        for i, (x, y, d, t, m1, m0) in enumerate(dataloader_tr):
            ...
            # model(x, d, m1)
            yhat, _, mhat = model([x, d.to(device),  m0]) # -- output:  y, x, m
            ...
            loss = loss1 + args.lambda_m * loss3 # 汇总所有的误差函数
            ...
            s = "epoch:{}-{} || 学习率: {:.3f} || 正样本比例 {:.3f}|| loss1: {:.3f} || loss3: {:.3f}"
            print(s.format(epoch + 1,\
                    i+1, lr_scheduler.get_last_lr()[0],p,loss1.cpu().data, loss3.cpu().data))
        ...
        with torch.no_grad():
            ...
            torch.save(model.state_dict(), "{}/new_model2/model_params/param-model2_1-ylabel{}-{}.pth".format(args.root, col,epoch + 1))
            print("模型已经保存在: {}/new_model2/model_params/param-model2_1-ylabel{}-{}.pth".format(args.root, col, epoch + 1))
