...
class Mynet(nn.Module):
    def __init__(self, model_config):
        ...
        self.output_x = nn.Sequential(  # 输出 x 的预测
        ...
        self.decay_x = nn.Sequential(  # 用来调和上一次观测和预测值
    ...
    def forward(self, inputs):
        ...
        for i1 in range(time_len):  # 沿着时间维度
            ...
            if i1 == 0: # 如果是第一次，就不需要缺失值替换
                ...
                gamma_x = self.decay_x(d[:, i1, :]) # (batch, 1) --> batch, input_size
                x1 = (x_hat*gamma_x + x[:,i1-1,:]*(1-gamma_x)) * (1-m1[:, i1, :]) + m1[:, i1, :] * x[:, i1, :]  # 缺失值替换
...
if __name__ == "__main__":
    ...
    for epoch in range(args.start_epoch, args.start_epoch+Epoch):
        ...
        for i, (x, y, d, t, m1, m0) in enumerate(dataloader_tr):
            ...
            count+=1
            if count>=50: # 当训练次数超过阈值，停止本次 epoch ，进入测试
                break
        ...
        with torch.no_grad():
            ...
            torch.save(model.state_dict(), "{}/new_model2/model_params/param-model7-ylabel{}-{}.pth".format(args.root, col,epoch + 1))
            print("模型已经保存在: {}/new_model2/model_params/param-model7-ylabel{}-{}.pth".format(args.root, col, epoch + 1))
