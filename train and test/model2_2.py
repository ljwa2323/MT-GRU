...
def get_argment():
    ...
    # 多少次循环 来衰减 一次 学习率
    parser.add_argument("--n-step", help="steps to decay the lr")
    # 从第几个循环开始训练
...
if __name__ == "__main__":
    ...
    for epoch in range(args.start_epoch, args.start_epoch+Epoch):
        ...
        count = 0 # 用于计数，若一个 epoch 中训练次数超过多少，就进入下一个 epoch
        ...
        for i, (x, y, d, t, m1, m0) in enumerate(dataloader_tr):
            ...
            # model(x, d, m1)
            yhat, xhat, _ = model([x, d.to(device),  m0]) # -- output:  y, x, m
            ...
            # loss3 = F.binary_cross_entropy_with_logits(mhat[m1[:,1:,col]==1].view(-1,1),  # mhat：(batch, time-1, var) 
                                                        ...
                                                        # ) 
            loss = loss1 + args.lambda_x * loss2  # 汇总所有的误差函数
            ...
            s = "epoch:{}-{} || 学习率: {:.3f} || 正样本比例 {:.3f}|| loss1: {:.3f} || loss2: {:.3f}"
            print(s.format(epoch + 1,\
                    i+1, lr_scheduler.get_last_lr()[0],p,loss1.cpu().data, loss2.cpu().data))
        ...
        with torch.no_grad():
            ...
            torch.save(model.state_dict(), "{}/new_model2/model_params/param-model2_2-ylabel{}-{}.pth".format(args.root, col,epoch + 1))
            print("模型已经保存在: {}/new_model2/model_params/param-model2_2-ylabel{}-{}.pth".format(args.root, col, epoch + 1))
