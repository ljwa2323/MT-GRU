...
def get_argment():
    ...
    # 输出的 x 的类型
    parser.add_argument("--x-file", help="type of input x", type=str, default="x1.csv")
...
if __name__ == "__main__":
    ...
    for epoch in range(args.start_epoch, args.start_epoch+Epoch):
        ...
        for i, (x, y, d, t, m1) in enumerate(dataloader_tr):
            ...
            optimizer.zero_grad()
            yhat = model([x, d.to(device)]) # -- output:  y
        ...
        with torch.no_grad():
            ...
            for i, (x, y, d, t, m1) in enumerate(dataloader_te):
                ...
                y = y.to(device)
                m1 = m1.to(device)
                yhat = model([x.to(device), d.to(device)]) # -- output:  y
            ...
            if args.note: # 如果有备注信息， 则将备注信息加在文件名最后
                torch.save(model.state_dict(), "{}/new_model2/model_params/param-model0-ylabel{}-{}-{}.pth".format(args.root, col,epoch + 1,args.note))
                print("模型已经保存在: {}/new_model2/model_params/param-model0-ylabel{}-{}-{}.pth".format(args.root, col, epoch + 1,args.note))
            else:
                torch.save(model.state_dict(), "{}/new_model2/model_params/param-model0-ylabel{}-{}.pth".format(args.root, col,epoch + 1))
                print("模型已经保存在: {}/new_model2/model_params/param-model0-ylabel{}-{}.pth".format(args.root, col, epoch + 1))
